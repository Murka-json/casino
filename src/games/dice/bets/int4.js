...
export default async (ctx, user, conv) => {
    if(ctx.isChat && ctx.messagePayload?.command == 'dice#int4') {
        if (conv.gameData.game == 'dice') {
            ...
            if (scale > 1) {
                _coin = await ctx.question({
                    message: `[id${ctx.senderId}|${user.name}], введи сумму ставки на число 4 ИЛИ нажми на кнопку:`,
            ...
            if (!conv.gameData.bets[ctx.senderId].int4) {
                conv.gameData.bets[ctx.senderId].int4 = 0
            }
            conv.gameData.bets[ctx.senderId].int4 += Math.floor(_coin)
            ...
            if (scale >= _coin) {
                ...
                if (Number(user.balance) >= Number(_coin)) {
                    ...
                    return ctx.send(`[id${ctx.senderId}|${user.name}], успешная ставка ${utils.number_format(_coin)} VkCoin на число 4`)
                }
