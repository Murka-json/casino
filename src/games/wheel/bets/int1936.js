...
export default async (ctx, user, conv) => {
    if(ctx.isChat && ctx.messagePayload?.command == 'wheel#1936') {
        if (conv.gameData.game == 'wheel') {
            if(conv.gameData.bets[ctx.senderId] && conv.gameData.bets[ctx.senderId].int118 > 0) return ctx.send(`[id${ctx.senderId}|${user.name}], вы уже поставили на 1-18!`)
            ...
            if (scale > 1) {
                _coin = await ctx.question({
                    message: `[id${ctx.senderId}|${user.name}], введи сумму ставки на 19-36 ИЛИ нажми на кнопку:`,
            ...
            if (!conv.gameData.bets[ctx.senderId].int1936) {
                conv.gameData.bets[ctx.senderId].int1936 = 0
            }
            conv.gameData.bets[ctx.senderId].int1936 += Math.floor(_coin)
            ...
            if (scale >= _coin) {
                ...
                if (Number(user.balance) >= Number(_coin)) {
                    ...
                    return ctx.send(`[id${ctx.senderId}|${user.name}], успешная ставка ${utils.number_format(_coin)} VkCoin на 19-36`)
                }
