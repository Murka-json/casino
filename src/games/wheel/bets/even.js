...
export default async (ctx, user, conv) => {
    if(ctx.isChat && ctx.messagePayload?.command == 'wheel#even') {
        if (conv.gameData.game == 'wheel') {
            if(conv.gameData.bets[ctx.senderId] && conv.gameData.bets[ctx.senderId].noEven > 0) return ctx.send(`[id${ctx.senderId}|${user.name}], вы уже поставили на нечётное!`)
            ...
            if (scale > 1) {
                _coin = await ctx.question({
                    message: `[id${ctx.senderId}|${user.name}], введи сумму ставки на четное ИЛИ нажми на кнопку:`,
            ...
            if (!conv.gameData.bets[ctx.senderId].even) {
                conv.gameData.bets[ctx.senderId].even = 0
            }
            conv.gameData.bets[ctx.senderId].even += Math.floor(_coin)
            ...
            if (scale >= _coin) {
                ...
                if (Number(user.balance) >= Number(_coin)) {
                    ...
                    return ctx.send(`[id${ctx.senderId}|${user.name}], успешная ставка ${utils.number_format(_coin)} VkCoin на четное`)
                }
