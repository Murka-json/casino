...
export default async (ctx, user, conv) => {
    if(ctx.isChat && ctx.messagePayload?.command == 'wheel#1324') {
        if (conv.gameData.game == 'wheel') {
            if(conv.gameData.bets[ctx.senderId] && conv.gameData.bets[ctx.senderId].int112 > 0 && conv.gameData.bets[ctx.senderId].int2536 > 0) return ctx.send(`[id${ctx.senderId}|${user.name}], вы можете ставить только на два промежутка!`)
            ...
            if (scale > 1) {
                _coin = await ctx.question({
                    message: `[id${ctx.senderId}|${user.name}], введи сумму ставки на 13-24 ИЛИ нажми на кнопку:`,
            ...
            if (!conv.gameData.bets[ctx.senderId].int1324) {
                conv.gameData.bets[ctx.senderId].int1324 = 0
            }
            conv.gameData.bets[ctx.senderId].int1324 += Math.floor(_coin)
            ...
            if (scale >= _coin) {
                ...
                if (Number(user.balance) >= Number(_coin)) {
                    ...
                    return ctx.send(`[id${ctx.senderId}|${user.name}], успешная ставка ${utils.number_format(_coin)} VkCoin на 13-24`)
                }
