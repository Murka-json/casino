...
export default async (ctx, user, conv) => {
    if(ctx.isChat && ctx.messagePayload?.command == 'wheel#black') {
        if (conv.gameData.game == 'wheel') {
            if(conv.gameData.bets[ctx.senderId] && conv.gameData.bets[ctx.senderId].red > 0) return ctx.send(`[id${ctx.senderId}|${user.name}], вы уже поставили на красное!`)
            ...
            if (scale > 1) {
                _coin = await ctx.question({
                    message: `[id${ctx.senderId}|${user.name}], введи сумму ставки на черное ИЛИ нажми на кнопку:`,
            ...
            if (!conv.gameData.bets[ctx.senderId].black) {
                conv.gameData.bets[ctx.senderId].black = 0
            }
            conv.gameData.bets[ctx.senderId].black += Math.floor(_coin)
            ...
            if (scale >= _coin) {
                ...
                if (Number(user.balance) >= Number(_coin)) {
                    ...
                    return ctx.send(`[id${ctx.senderId}|${user.name}], успешная ставка ${utils.number_format(_coin)} VkCoin на черное`)
                }
