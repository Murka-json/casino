...
export default async (ctx, user, conv) => {
    if(ctx.isChat && ctx.messagePayload?.command == 'wheel#number') {
        if (conv.gameData.game == 'wheel') {
            ...
            if (!conv.gameData.bets[ctx.senderId].numbers) {
                conv.gameData.bets[ctx.senderId].numbers = {}
            }
            if(!conv.gameData.bets[ctx.senderId].numbers[number.text]) {
                conv.gameData.bets[ctx.senderId].numbers[number.text] = {
                    amount: 0,
                    number: number.text
                }
            }
            conv.gameData.bets[ctx.senderId].numbers[number.text].amount += Math.floor(_coin)
            ...
            if (scale >= _coin) {
                ...
                if (Number(user.balance) >= Number(_coin)) {
                    ...
                    return ctx.send(`[id${ctx.senderId}|${user.name}], успешная ставка ${utils.number_format(_coin)} VkCoin на число ${number.text}`)
                }
