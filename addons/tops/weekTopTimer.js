    ...
    if (day = 0 && o == '00:00') {
        ...
        for (let i in db.usersData) {
            top.push({
                ...
                win: db.usersData[i].winWeek
            })
        }

        for(let i in db.convsData) {
            db.convsData[i].gameData.procentAmount.week = 0
        }
        ...
        for (let j in top) {
            ...
            if(topS == 1) {
                db.usersData[top[j].id].bbalance += Number(config.weekTopPrizes[topS - 1])
                db.usersData[top[j].id].winWeek = 0
                vk.api.messages.send({
                    peer_id: top[j].id,
                    message: `🔥 Вы заняли ${topS} место в еженедельном рейтинге игроков и получили ${utils.number_format(config.weekTopPrizes[topS - 1])} VkCoin!`,
